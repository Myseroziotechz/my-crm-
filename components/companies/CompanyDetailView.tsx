...
import type { Call, Column, Company, Contact, Lead } from "@/lib/types";
...
export function CompanyDetailView({
  ...
  const leadCols: Column<Lead>[] = [
    { key: "id", header: "Lead ID", render: (l) => <span className="font-mono text-xs text-ink-500">{l.id.slice(0, 8)}</span> },
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        <Card className="lg:col-span-1">
          ...
          <CardBody className="space-y-3">
            {[
              ...
              [MapPin, "Address", [company.address, company.city, company.state].filter(Boolean).join(", ") || "—"],
        ...
        <Card className="lg:col-span-2">
          ...
          <div className="p-4">
                ...
                <div>
                  ...
                  {contacts.find((c) => c.isPrimary) ? (
                    <p className="text-sm text-ink-600">
                      ...
                      {contacts.find((c) => c.isPrimary)!.designation}
                    </p>
                  ) : (
            ...
            {tab === "contacts" && (
              <DataTable columns={contactCols} rows={contacts} emptyTitle="No contacts" />
            )}
            {tab === "leads" && (
              <DataTable
                columns={leadCols}
            ...
            {tab === "calls" && (
              <DataTable columns={callCols} rows={calls} emptyTitle="No calls logged" />
            )}
            {tab === "activities" && (
